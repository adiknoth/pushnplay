...
static gchar *green = "green";        /* Colour to be used for "green" timelabel */
static gchar *yellow = "yellow";      /* Colour to be used for "yellow" timelabel */
static gchar *red = "red";            /* Colour to be used for "red" timelabel */
static gchar *conffile = "~/.pushnplay";            /* Default config file */
...
static void _set_button_color (GtkWidget *button, const gchar *bg_color,
		gboolean fg) {
	GdkRGBA color;
	...
	if (TRUE == fg) {
		...
		gtk_widget_override_color (button, GTK_STATE_NORMAL, &color);
	}
}

static void set_default_button_color (CustomData *data) {
	_set_button_color(data->playPauseButton, data->config_button_color,
			TRUE);
	_set_button_color(data->playPauseButton, data->config_button_fg_color,
			FALSE);
}
...
static GtkWidget* create_player_ui (CustomData *data, guint decknumber) {
     ...
     */
    gtk_widget_set_hexpand (data->playPauseButton, TRUE);
    gtk_widget_set_vexpand (data->playPauseButton, TRUE);


    data->timelabel = gtk_label_new ("");
    update_timelabel(data, "Time remaining");

    gtk_grid_attach (GTK_GRID (myGrid), data->playPauseButton, 0, 0, 1, 1);
    gtk_grid_attach_next_to (GTK_GRID (myGrid), data->timelabel, data->playPauseButton, GTK_POS_BOTTOM, 1, 1);
...
static gboolean refresh_ui (CustomData *data) {
...
#endif
    {
		...
		/* Make the button blink while playing*/
		if (data->blink) {
			set_default_button_color(data);
		} else {
			_set_button_color(data->playPauseButton,
					"lighter (@bg_color)", TRUE);
		}
...
static GtkWidget* create_mainwindow(void) {
    ...
    gtk_window_set_title (GTK_WINDOW (main_window), "pushnplay");
...
static gchar * _config_readline(GDataInputStream *config) {
	GError *error = NULL;
	gsize length;
	gchar *result;

	result = g_data_input_stream_read_line_utf8(config, &length, NULL, &error);
	if (NULL != error) {
...
static gchar * _read_color_from_config (GDataInputStream *config) {
	gchar *result = _config_readline(config);
	if (NULL == result) {
		result = g_strdup ("lighter (@bg_color)");
	}
	return result;
}
...
static void _configfile (CustomData *data, gboolean writemode) {
    ...
    if (!g_strcmp0(conffile, "~/.pushnplay")) {
      filename = g_strdup_printf("file://%s/.pushnplay", g_get_home_dir());
    } else {
      filename = g_strdup_printf("file:///%s", conffile);
    }
    ...
    if (TRUE == writemode) {
        ...
        for (int i = 0; i < num_players; i++) {
            data[i].config_audio_uri = _config_readline(config);
            data[i].config_button_text = _config_readline(config);
            data[i].config_button_color = _read_color_from_config(config);
            data[i].config_button_fg_color = _read_color_from_config(config);
            g_print ("Will use %s for deck %i\n", data[i].config_audio_uri, i);
        }
...
int main(int argc, char *argv[]) {
    ...
    GOptionEntry option_entries[] = {
        ...
        { "conffile", 'c', G_OPTION_FLAG_IN_MAIN, G_OPTION_ARG_FILENAME,
            &conffile, "Absolute path to config file", "~/.pushnplay" },
        { "columns", 0, G_OPTION_FLAG_IN_MAIN, G_OPTION_ARG_INT,
    ...
    for (int i=0; i < num_players; i++) {
        ...
        /* num_x columns (i%num_x) in (i/num_x) rows */
        gtk_grid_attach (GTK_GRID (main_grid), playerUI, i % num_x, i / num_x, 1, 1);
    }
...

    {
            ...
            for (int i=0; i < num_players; i++) {
		    gtk_button_set_label (GTK_BUTTON (data[i].playPauseButton), data[i].config_button_text);

                    audio_set_uri(&data[i], data[i].config_audio_uri);
                    audio_pause_player(&data[i]);

		    set_default_button_color (&data[i]);
